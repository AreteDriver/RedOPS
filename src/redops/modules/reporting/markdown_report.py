...
def generate_exec_summary(ctx: Context, params: Optional[Dict[str, Any]] = None) -> Context:
    """
    ...
    # Save to file
    output_path = output_dir / f"executive_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def generate_technical_report(ctx: Context, params: Optional[Dict[str, Any]] = None) -> Context:
    """
    ...
    # Save to file
    output_path = output_dir / f"technical_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    with open(output_path, 'w') as f:
        f.write(report)
