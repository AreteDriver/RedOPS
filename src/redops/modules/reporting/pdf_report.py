...
class RedOPSPDFReport(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            10,
            "RedOPS Security Assessment Report",
            align="C",
            new_x=XPos.LMARGIN,
            new_y=YPos.NEXT,
        )
    ...
    def add_finding(
        ...
        if recommendation:
            ...
            rec_text = (
                recommendation[:300] if len(recommendation) > 300 else recommendation
            )
...
class PDFReportGenerator:
    ...
    def _add_title_page(self, pdf: RedOPSPDFReport, data: dict[str, Any]):
        ...
        pdf.cell(
            0, 10, f"Target: {target}", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT
        )
        ...
        pdf.cell(
            0,
            10,
            "Generated by RedOPS Security Framework",
            align="C",
            new_x=XPos.LMARGIN,
            new_y=YPos.NEXT,
        )
    ...
    def _add_recommendations(self, pdf: RedOPSPDFReport, data: dict[str, Any]):
        ...
        if not recommendations:
            pdf.body_text(
                "Based on the assessment, consider the following general recommendations:"
            )
    ...
    def _add_appendix(self, pdf: RedOPSPDFReport, data: dict[str, Any]):
        ...
        pdf.body_text(
            f"Assessment Date: {data.get('timestamp', 'Not recorded')[:10] if data.get('timestamp') else 'Not recorded'}"
        )
