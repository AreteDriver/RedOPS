...
class PDFReportGenerator:
    ...
    def _add_title_page(self, pdf: RedOPSPDFReport, data: Dict[str, Any]):
        ...
        pdf.ln(40)
        pdf.cell(0, 20, "Security Assessment Report", align="C", ln=True)

        pdf.set_font("Helvetica", "", 14)
        pdf.ln(10)
        target = data.get("target", "Unknown Target")
        pdf.cell(0, 10, f"Target: {target}", align="C", ln=True)

        pdf.ln(5)
        timestamp = data.get("timestamp", datetime.now(timezone.utc).isoformat())
        pdf.cell(0, 10, f"Date: {timestamp[:10]}", align="C", ln=True)

        pdf.ln(20)
        pdf.set_font("Helvetica", "I", 10)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 10, "Generated by RedOPS Security Framework", align="C", ln=True)
    ...
    def _add_recommendations(self, pdf: RedOPSPDFReport, data: Dict[str, Any]):
        ...
        for i, rec in enumerate(recommendations[:10], 1):
            ...
            pdf.cell(0, 5, f"Priority: {priority}", ln=True)
            pdf.ln(3)
