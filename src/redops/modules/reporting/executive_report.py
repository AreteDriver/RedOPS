...
def generate_executive_report(
    ...
    summary = ExecutiveSummary(
        ...
        top_risks=top_risks,
        compliance_summary=compliance_summary.to_dict() if hasattr(compliance_summary, 'to_dict') else compliance_summary,
    ...
    report = {
        ...
        "compliance_report": compliance_summary.to_dict() if hasattr(compliance_summary, 'to_dict') else compliance_summary,
...
def calculate_risk_dashboard(
    ...
    max_possible = len(findings) * 12.0 if findings else 1.0  # Max: critical * credential
    ...
    # Exposure metrics
    exposure_metrics = {
        "total_exposures": severity_counts.get("critical", 0) + severity_counts.get("high", 0),
        "public_facing": sum(1 for f in findings if "exposure" in f.get("category", "")),
        "credential_risks": sum(1 for f in findings if "credential" in f.get("category", "")),
    }
...
def generate_compliance_summary(
    ...
    assessments = compliance_data.get("assessments", [])
    ...
    for gap in sorted(gaps, key=lambda g: severity_priority(g.get("priority", "low")))[:5]:
...
def generate_recommendations(
    ...
    for finding in critical_findings[:5]:
        ...
        recommendations.append(Recommendation(
            ...
            priority="critical",
            category=category,
            business_impact=BUSINESS_IMPACT_TEMPLATES.get(category, "Security risk"),
            effort="high" if category in ["vulnerability", "infrastructure"] else "medium",
            timeline="immediate",
...
def calculate_executive_metrics(
    ...
    return {
        ...
        "critical_high_ratio": round(critical_high / total_findings * 100, 1) if total_findings else 0,
        ...
        "controls_assessed": controls_assessed,
        "controls_passing": controls_passing,
        "compliance_rate": round(controls_passing / controls_assessed * 100, 1) if controls_assessed else 0,
    }
...
def get_key_findings(findings: List[Dict[str, Any]], limit: int = 5) -> List[Dict[str, Any]]:
...
def get_top_risks_from_findings(
    ...
    for finding in findings:
        ...
        score = SEVERITY_WEIGHTS.get(severity, 1.0) * CATEGORY_WEIGHTS.get(category, 1.0)
...
The assessment identified {finding_count} key findings requiring attention. """
    ...
    if critical_count > 0:
        brief += f"There are {critical_count} critical issues requiring immediate action. "
...
def generate_next_steps(
    ...
    if high_recs:
        next_steps.append(f"Schedule remediation for {len(high_recs)} high-priority items")
