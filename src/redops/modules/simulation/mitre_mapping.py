...
def generate_navigator_layer(
    techniques: set[str],
    name: str = "RedOPS Scan Results",
    description: str = "MITRE ATT&CK coverage generated by RedOPS",
) -> dict[str, Any]:
    """
    Generate a MITRE ATT&CK Navigator layer JSON.

    Output is compatible with MITRE ATT&CK Navigator (https://mitre-attack.github.io/attack-navigator/).

    Args:
        techniques: Set of technique IDs to highlight
        name: Layer name
        ...
        layer_techniques.append(
            {
                ...
                "score": 1,
                "comment": f"{technique.name}: {technique.description}",
                "enabled": True,
            }
        )
    ...
    return {
        ...
        "legendItems": [
            {
                "label": "Technique detected by RedOPS",
                "color": "#dc3545",
            }
        ],
        "showTacticRowBackground": True,
        "tacticRowBackground": "#eeeeee",
