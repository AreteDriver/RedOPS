...
class DataImporter(ABC):
    ...
    @abstractmethod
    def import_stream(self, stream: Union[TextIO, BinaryIO]) -> ImportResult:
...
class JSONLImporter(DataImporter):
    ...
    def iter_records(self, path: Union[str, Path]) -> Generator[Dict, None, None]:
...
def import_json(path: Union[str, Path], **kwargs) -> Tuple[List[Dict], ImportResult]:
    """Import JSON file."""
    records = []
...
def import_jsonl(path: Union[str, Path], **kwargs) -> Tuple[List[Dict], ImportResult]:
...
def import_csv(path: Union[str, Path], **kwargs) -> Tuple[List[Dict], ImportResult]:
    """Import CSV file."""
    records = []
...
def import_stix(path: Union[str, Path], **kwargs) -> Tuple[List[Dict], ImportResult]:
    """Import STIX file."""
    records = []
...
def export_json(records: List[Dict], path: Union[str, Path], **kwargs) -> ExportResult:
    """Export to JSON file."""
    exporter = JSONExporter(**kwargs)
    return exporter.export_file(records, path)


def export_jsonl(records: List[Dict], path: Union[str, Path], **kwargs) -> ExportResult:
    """Export to JSON Lines file."""
    exporter = JSONLExporter(**kwargs)
    return exporter.export_file(records, path)


def export_csv(records: List[Dict], path: Union[str, Path], **kwargs) -> ExportResult:
    """Export to CSV file."""
    exporter = CSVExporter(**kwargs)
    return exporter.export_file(records, path)


def export_stix(records: List[Dict], path: Union[str, Path], **kwargs) -> ExportResult:
    """Export to STIX file."""
