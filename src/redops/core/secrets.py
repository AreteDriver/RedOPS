...
class SimpleEncryption(EncryptionProvider):
    """
    Simple XOR-based encryption for environments without cryptography.

    WARNING: This is NOT cryptographically secure and should only be
    used for testing or when cryptography is unavailable.
    """

    # Magic marker for integrity check
    _MARKER = b"SENC"
    ...
    def _compute_checksum(self, data: bytes) -> bytes:
        """Compute a simple 4-byte checksum."""
        return hashlib.md5(data).digest()[:4]

    def encrypt(self, plaintext: str) -> str:
        """Encrypt using XOR with integrity check."""
        data = plaintext.encode()
        checksum = self._compute_checksum(data)
        # Format: MARKER + checksum + data
        payload = self._MARKER + checksum + data
        encrypted = self._xor_bytes(payload, self._key)
        return base64.urlsafe_b64encode(encrypted).decode()

    def _try_decrypt(self, encrypted: bytes, key: bytes) -> Optional[str]:
        """Try to decrypt with a key, return None if integrity check fails."""
        try:
            decrypted = self._xor_bytes(encrypted, key)
            # Verify marker
            if not decrypted.startswith(self._MARKER):
                return None
            # Verify checksum
            checksum = decrypted[4:8]
            data = decrypted[8:]
    ...
    def decrypt(self, ciphertext: str) -> str:
        """Decrypt using XOR with integrity verification."""
        encrypted = base64.urlsafe_b64decode(ciphertext.encode())

        # Try current key first
        result = self._try_decrypt(encrypted, self._key)
        if result is not None:
            return result

        # Try previous keys (newest first)
        for prev_key in reversed(self._previous_keys):
            result = self._try_decrypt(encrypted, prev_key)
            if result is not None:
                return result
    ...
    def rotate_key(self) -> None:
        """Generate a new key."""
        self._previous_keys.append(self._key)
        self._key = base64.b64encode(os.urandom(32)).decode().encode()
