...
class PipelineStep(BaseModel):
    ...
    module: str = Field(description="Dotted path to module function, e.g., 'recon.domains.profile_domain'")
    params: Dict[str, Any] = Field(default_factory=dict, description="Parameters to pass to the module")
    enabled: bool = Field(default=True, description="Whether this step is enabled")
    continue_on_error: bool = Field(default=False, description="Continue pipeline execution if this step fails")
...
class Pipeline(BaseModel):
    ...
    steps: List[PipelineStep] = Field(min_length=1, description="Pipeline must have at least one step")
